...
public class RocketMQAutoConfiguration {
    ...
    public static class ListenerContainerConfiguration implements ApplicationContextAware, InitializingBean {
        ...
        private void registerContainer(String beanName, Object bean) {
            ...
            if(!"*".equals(annotation.consumeTime())){
                String time = environment.resolvePlaceholders(annotation.consumeTime());
                SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                Date dateTime = null;
                try {
                    dateTime = simpleDateFormat.parse(time);
                    beanBuilder.addPropertyValue(PROP_CONSUMER_TIMESTAMP, UtilAll.timeMillisToHumanString3(dateTime.getTime()));
                } catch (ParseException e) {
                    log.error("consumeTime is not this format of yyyy-MM-dd HH:mm:ss",e);
                }
            }
